...
public static class WorldPersistence
{
    ...
    /// <summary>
    /// Replaces <paramref name="world"/>'s contents with the contents of
    /// <paramref name="save"/>. Architecture Hardening A6 relocated this
    /// orchestration from <c>CityWorld.Restore</c> into the persistence
    /// layer; this facade keeps every caller from having to know about
    /// <c>WorldSaveApplier</c> directly.
    /// </summary>
    public static void ApplyTo(CityWorld world, WorldSave save) =>
        WorldSaveApplier.ApplyTo(world, save);

    /// <summary>
    /// Builds a fresh <see cref="CityWorld"/> from a validated snapshot.
    /// A6 relocated the implementation from <c>CityWorld.FromSave</c>;
    /// the facade preserves the old entry point so Presentation and tests
    /// keep calling <c>WorldPersistence.FromSave(save)</c>.
    /// </summary>
    public static CityWorld FromSave(WorldSave save) =>
        WorldSaveApplier.FromSave(save);
    ...
    public static WorldSave Capture(CityWorld world, DateTimeOffset now)
    {
        ...
        {
            save.CityInventory[ResourceTypeSaveIds.ToId(resource)] = amount;
        }
        foreach (ToolKind tool in world.Tools.OrderBy(tool => tool))
        {
            save.Tools.Add(ToolKindSaveIds.ToId(tool));
        }
        ...
        if (world.FirstNight is { } firstNight)
        {
            ...
            {
                Stage = FirstNightStageSaveIds.ToId(firstNight.Stage),
        ...
        foreach (var building in world.Buildings.Values)
        {
            var bs = new BuildingSave
            {
                ...
                Kind = BuildingKindSaveIds.ToId(building.Kind),
                ProducedResourceType = ResourceTypeSaveIds.ToId(building.ProducedResourceType),
        ...
        foreach (CityParcel parcel in world.Parcels.Values)
        {
            save.Parcels.Add(new ParcelSave
            {
                ...
                TerritoryState = ParcelTerritoryStateSaveIds.ToId(parcel.TerritoryState),
            });
        }
        foreach (NaturalResourcePatch patch in world.NaturalResourcePatches.Values)
        {
            var patchSave = new NaturalResourcePatchSave
            {
                ...
                ResourceType = ResourceTypeSaveIds.ToId(patch.ResourceType),
        ...
        foreach (ParcelPlacement placement in world.ParcelPlacements.Values)
        {
            save.ParcelPlacements.Add(new ParcelPlacementSave
            {
                ...
                Orientation = BuildingOrientationSaveIds.ToId(placement.Orientation),
            });
        }
        ...
        foreach (var citizen in world.Citizens.Values)
        {
            var cs = new CitizenSave
            {
                ...
                Origin = CitizenOriginSaveIds.ToId(citizen.Origin),
                ...
                CommitmentKind = CitizenCommitmentKindSaveIds.ToId(citizen.Commitment.Kind),
                CommitmentEntityId = citizen.Commitment.EntityId,
                WorkOrderKind = citizen.WorkOrder is { } wo
                    ? CitizenCommitmentKindSaveIds.ToId(wo.Kind)
                    : null,
                WorkOrderEntityId = citizen.WorkOrder?.TargetId.Value,
                VitalStatus = CitizenVitalStatusSaveIds.ToId(citizen.VitalStatus),
                TransitStartedAtTick = citizen.TransitStartedAtTick,
                CurrentLocation = CitizenLocationSaveIds.ToId(citizen.CurrentLocation),
                ResumeWorkNotBeforeTick = citizen.ResumeWorkNotBeforeTick,
                IsReturningHome = citizen.IsReturningHome,
                WoundSeverity = citizen.Wound is { } w
                    ? WoundSeveritySaveIds.ToId(w.Severity)
                    : null,
            ...
            foreach (CompetencyProgress progress in citizen.WeaponCompetencies.Values)
            {
                ...
                {
                    Family = WeaponFamilySaveIds.ToId(progress.Family),
        ...
        foreach (var project in world.Projects.Values)
        {
            ...
            {
                Id = project.Id.Value,
                Kind = ConstructionKindSaveIds.ToId(project.Kind),
                ...
                ActiveFoundingModule = project.ActiveFoundingModule is { } am
                    ? FoundingSiteModuleSaveIds.ToId(am)
                    : null,
            ...
            {
                ps.DepositedInputs[ResourceTypeSaveIds.ToId(input.Resource)] = input.Amount;
            }
            foreach (RecipeInput input in project.RemainingInputs)
            {
                ps.RemainingInputs[ResourceTypeSaveIds.ToId(input.Resource)] = input.Amount;
            }
        ...
        foreach (CultivationSite site in world.CultivationSites.Values)
        {
            ...
            {
                Id = site.Id.Value,
                State = CultivationPlotStateSaveIds.ToId(site.State),
        ...
        foreach (var evt in retained)
        {
            save.Events.Add(new WorldEventSave
            {
                Id = evt.Id.Value,
                Tick = evt.Tick,
                Kind = WorldEventKindSaveIds.ToId(evt.Kind),
                SubjectKind = WorldEventSubjectKindSaveIds.ToId(evt.Subject.Kind),
        ...
        foreach (ResourceReservation reservation in world.Resources.Reservations)
        {
            save.ResourceReservations.Add(new ResourceReservationSave
            {
                ...
                Resource = ResourceTypeSaveIds.ToId(reservation.Resource),
                Amount = reservation.Amount,
                OwnerKind = ResourceReservationOwnerKindSaveIds.ToId(reservation.Owner.Kind),
        ...
        foreach (ResourceOpportunity opportunity in world.ResourceOpportunities.Values)
        {
            save.ResourceOpportunities.Add(new ResourceOpportunitySave
            {
                ...
                Kind = ResourceOpportunityKindSaveIds.ToId(opportunity.Kind),
                State = ResourceOpportunityStateSaveIds.ToId(opportunity.State),
        ...
        foreach (Expedition expedition in world.Expeditions.Values)
        {
            ...
            save.Expeditions.Add(new ExpeditionSave
            {
                ...
                RewardResource = expedition.RewardResource is { } rr
                    ? ResourceTypeSaveIds.ToId(rr)
                    : null,
                RewardAmount = expedition.RewardAmount,
                ReservationId = expedition.ReservationId?.Value,
                Status = ExpeditionStatusSaveIds.ToId(expedition.Status),
                ReturnedAmount = expedition.ReturnedAmount,
                RewardKind = ExpeditionRewardKindSaveIds.ToId(expedition.RewardKind),
                DeliveredMigrantId = expedition.DeliveredMigrantId?.Value,
                Phase = ExpeditionPhaseSaveIds.ToId(expedition.Phase),
                EncounterOutcome = expedition.EncounterOutcome is { } eo
                    ? ExpeditionEncounterOutcomeSaveIds.ToId(eo)
                    : null,
                RetreatPosture = ExpeditionRetreatPostureSaveIds.ToId(expedition.RetreatPosture),
                ...
                ResourceOpportunityKind = expedition.ResourceOpportunityKind is { } rok
                    ? ResourceOpportunityKindSaveIds.ToId(rok)
                    : null,
                ...
                CombatCommands = combatSession?.Commands
                    .Select(command => new CombatSessionCommandSave
                    {
                        ...
                        Kind = CombatSessionCommandKindSaveIds.ToId(command.Kind),
                        Value = command.Value,
                    })
    ...
    internal static EarlyGameMetricsSave CaptureEarlyGameMetrics(EarlyGameMetrics metrics)
    {
        ...
        {
            save.Gathered[ResourceTypeSaveIds.ToId(entry.Key)] = entry.Value;
        }
        foreach (KeyValuePair<ResourceType, int> entry in metrics.Consumed)
        {
            save.Consumed[ResourceTypeSaveIds.ToId(entry.Key)] = entry.Value;
        }
        return save;
    }
        ...
        EarlyGameMetricsSave? save)
    {
        ...
        {
            if (ResourceTypeSaveIds.TryParse(entry.Key, out ResourceType resource))
            {
    ...
    internal static CitizenProfileSave CaptureProfile(CitizenProfile profile)
    {
        var save = new CitizenProfileSave
        {
            ...
            Gender = GenderIdSaveIds.ToId(profile.Gender),
        ...
        {
            save.ElementalAffinity = ElementalAffinityIdSaveIds.ToId(
                CitizenProfile.ToLegacyAffinity(onboarding.ElementalAffinity));
    ...
    internal static CitizenProfile RestoreProfile(CitizenProfileSave save)
    {
        ...
        if (!string.IsNullOrEmpty(save.Gender)
            && GenderIdSaveIds.TryParse(save.Gender, out GenderId parsed))
        {
            gender = parsed;
        }
    ...
    internal static EquipmentLoadoutSave CaptureEquipmentLoadout(EquipmentLoadout loadout) => new()
    {
        ...
        {
            Family = WeaponFamilySaveIds.ToId(loadout.Weapon.Family),
    ...
    public static void Validate(WorldSave save)
    {
        ...
        if (save.Tools is null
            ...
            || save.Tools.Any(tool => !ToolKindSaveIds.TryParse(tool, out ToolKind parsed)
        ...
        {
            if (!FirstNightStageSaveIds.TryParse(firstNight.Stage, out FirstNightStage stage)
        ...
        if (save.CityInventory is null
            || save.CityInventory.Any(pair =>
                pair.Value < 0
                || !ResourceTypeSaveIds.TryParse(pair.Key, out ResourceType _)))
        {
        ...
        foreach (NaturalResourcePatchSave patch in save.NaturalResourcePatches)
        {
            if (patch is null
                ...
                || !ResourceTypeSaveIds.TryParse(patch.ResourceType, out ResourceType _)
                ...
                || (patch.ResourceType == ResourceTypeSaveIds.WoodId
        ...
        foreach (ParcelPlacementSave placement in save.ParcelPlacements)
        {
            if (placement is null
                ...
                || !BuildingOrientationSaveIds.TryParse(
        ...
        foreach (var evt in save.Events)
        {
            ...
            if (!WorldEventKindSaveIds.TryParse(evt.Kind, out WorldEventKind kind)
                || !WorldEventRetention.IsSignificant(kind))
            {
                throw new InvalidOperationException($"Event {evt.Id}: kind is not persistible.");
            }
            if (!WorldEventSubjectKindSaveIds.TryParse(evt.SubjectKind, out WorldEventSubjectKind subjectKind)
        ...
        foreach (ResourceReservationSave reservation in save.ResourceReservations)
        {
            if (reservation is null
                ...
                || !ResourceTypeSaveIds.TryParse(reservation.Resource, out ResourceType resource)
                || !ResourceReservationOwnerKindSaveIds.TryParse(reservation.OwnerKind, out ResourceReservationOwnerKind ownerKind))
            {
        ...
        foreach (ResourceOpportunitySave opportunity in save.ResourceOpportunities)
        {
            if (opportunity is null
                ...
                || !ResourceOpportunityKindSaveIds.TryParse(
                    opportunity.Kind,
                    out ResourceOpportunityKind _)
                || !ResourceOpportunityStateSaveIds.TryParse(
                    opportunity.State,
        ...
        foreach (ExpeditionSave expedition in save.Expeditions)
        {
            ...
            bool rewardKindParsed = ExpeditionRewardKindSaveIds.TryParse(
            ...
            if (expedition.Id <= 0
                ...
                || !ExpeditionStatusSaveIds.TryParse(expedition.Status, out ExpeditionStatus _)
                || !ExpeditionPhaseSaveIds.TryParse(expedition.Phase, out ExpeditionPhase _)
                || !ExpeditionRetreatPostureSaveIds.TryParse(expedition.RetreatPosture, out ExpeditionRetreatPosture _)
                || (!string.IsNullOrEmpty(expedition.EncounterOutcome)
                    && !ExpeditionEncounterOutcomeSaveIds.TryParse(expedition.EncounterOutcome, out ExpeditionEncounterOutcome _))
            ...
            foreach (CombatSessionCommandSave command in expedition.CombatCommands)
            {
                if (command is null
                    ...
                    || !CombatSessionCommandKindSaveIds.TryParse(command.Kind, out CombatSessionCommandKind kind)
            ...
            _ = ExpeditionPhaseSaveIds.TryParse(expedition.Phase, out ExpeditionPhase phase);
            ...
            ExpeditionEncounterOutcome? encounterOutcome =
                !string.IsNullOrEmpty(expedition.EncounterOutcome)
                && ExpeditionEncounterOutcomeSaveIds.TryParse(
            ...
            bool isSpiritTrail = string.Equals(
                expedition.ResourceOpportunityKind,
                ResourceOpportunityKindSaveIds.SpiritTrailSearchId,
        ...
        foreach (ResourceOpportunitySave opportunity in save.ResourceOpportunities)
        {
            ...
            if (expedition is null
                ...
                || !string.Equals(
                    expedition.Status,
                    ExpeditionStatusSaveIds.ActiveId,
                    StringComparison.OrdinalIgnoreCase))
            {
                throw new InvalidOperationException(
        ...
        foreach (var b in save.Buildings)
        {
            ...
            {
                if (!FoundingSiteModuleSaveIds.TryParse(module, out FoundingSiteModule _))
                {
                    throw new InvalidOperationException(
            ...
            if (b.FoundingSiteOriginModules.Count > 0
                && !string.Equals(b.Kind, BuildingKindSaveIds.HomeId, StringComparison.OrdinalIgnoreCase))
            {
                throw new InvalidOperationException(
        ...
        foreach (var c in save.Citizens)
        {
            ...
            if (hasWoundSeverity)
            {
                if (!WoundSeveritySaveIds.TryParse(c.WoundSeverity, out WoundSeverity woundSeverity)
        ...
        foreach (var pair in reservedByResource)
        {
            int cityStored = save.CityInventory
                .Where(entry => ResourceTypeSaveIds.TryParse(
                    entry.Key,
            ...
            int buildingStored = pair.Key == ResourceType.Iron
                ...
                : save.Buildings
                    .Where(building => ResourceTypeSaveIds.TryParse(building.ProducedResourceType, out ResourceType produced)
                        && produced == pair.Key)
        ...
        foreach (var p in save.Projects)
        {
            ...
            if (!ConstructionKindSaveIds.TryParse(p.Kind, out ConstructionKind projectKind))
            {
                throw new InvalidOperationException(
            ...
            if (string.Equals(p.Kind, ConstructionKindSaveIds.FoundingSiteId, StringComparison.OrdinalIgnoreCase))
            {
                var completedModules = new HashSet<FoundingSiteModule>();
                foreach (string module in p.CompletedFoundingModules)
                {
                    if (!FoundingSiteModuleSaveIds.TryParse(module, out FoundingSiteModule parsedModule))
                    {
                        throw new InvalidOperationException(
                ...
                {
                    if (!FoundingSiteModuleSaveIds.TryParse(p.ActiveFoundingModule, out FoundingSiteModule parsedActive))
                    {
                        throw new InvalidOperationException(
        ...
        foreach (CultivationSiteSave site in save.CultivationSites)
        {
            ...
            if (site.Id <= 0
                ...
                || !CultivationPlotStateSaveIds.TryParse(site.State, out CultivationPlotState state)
        ...
        int cultivationProjectCount = save.Projects.Count(project =>
            ConstructionKindSaveIds.TryParse(project.Kind, out ConstructionKind kind)
        ...
        {
            if (building.Kind != BuildingKindSaveIds.ForestId
        ...
        foreach (var c in save.Citizens)
        {
            ...
            List<ExpeditionSave> activeExpeditions = save.Expeditions
                .Where(expedition => expedition.MemberCitizenIds.Contains(c.Id)
                    && ExpeditionStatusSaveIds.TryParse(expedition.Status, out ExpeditionStatus status)
                    && status == ExpeditionStatus.Active)
                .ToList();
            ...
            if (!hasCommitmentKind
                || !CitizenCommitmentKindSaveIds.TryParse(
                    c.CommitmentKind,
    ...
    public static WorldSave MigrateV3ToV4(WorldSave save)
    {
        ...
        foreach (var cs in save.Citizens)
        {
            ...
            {
                cs.Profile.Gender = GenderIdSaveIds.MasculineId;
            }
        }
    ...
    public static WorldSave MigrateV7ToV8(WorldSave save)
    {
        ...
        foreach (BuildingSave building in save.Buildings)
        {
            if (building.Kind != BuildingKindSaveIds.ForestId) continue;
            save.Parcels.Add(new ParcelSave
            {
                Id = parcelId,
            ...
            save.NaturalResourcePatches.Add(new NaturalResourcePatchSave
            {
                Id = building.Id,
                ParcelId = parcelId,
                ResourceType = ResourceTypeSaveIds.WoodId,
    ...
    public static WorldSave MigrateV8ToV9(WorldSave save)
    {
        ...
        {
            if (building.Kind == BuildingKindSaveIds.ForestId) continue;
            BuildingKind kind = BuildingKindSaveIds.TryParse(
                building.Kind,
        ...
        foreach (ConstructionProjectSave project in save.Projects)
        {
            ConstructionKind kind = ConstructionKindSaveIds.TryParse(
                project.Kind,
        ...
        for (int index = 0; index < entities.Count; index++)
        {
            ...
            save.ParcelPlacements.Add(new ParcelPlacementSave
            {
                ...
                Orientation = BuildingOrientationSaveIds.SouthId,
            });
        }
        save.Version = 9;
        return save;
    ...
    public static WorldSave MigrateV13ToV14(WorldSave save)
    {
        ...
        save.Buildings.RemoveAll(building =>
            string.Equals(building.Kind, BuildingKindSaveIds.ForestId,
                StringComparison.OrdinalIgnoreCase));
        save.Version = 14;
        return save;
    }
        ...
        foreach (ExpeditionSave expedition in save.Expeditions)
        {
            ...
            if (string.IsNullOrEmpty(expedition.Phase))
            {
                bool stillActive = string.Equals(
                    expedition.Status, ExpeditionStatusSaveIds.ActiveId, StringComparison.OrdinalIgnoreCase);
                expedition.Phase = stillActive
                    ? ExpeditionPhaseSaveIds.OutboundId
                    : ExpeditionPhaseSaveIds.ResolvedId;
            }
        }
        save.Version = 16;
        return save;
    ...
    public static WorldSave MigrateV17ToV18(WorldSave save)
    {
        ...
        foreach (ExpeditionSave expedition in save.Expeditions)
        {
            ...
            WorldEventSave? dispatch = save.Events.LastOrDefault(evt =>
                ...
                && string.Equals(
                    evt.Kind,
                    WorldEventKindSaveIds.ExpeditionDispatchedId,
                    StringComparison.OrdinalIgnoreCase)
    ...
    public static WorldSave MigrateV28ToV29(WorldSave save)
    {
        ...
        foreach (CitizenSave citizen in save.Citizens)
        {
            if (!string.Equals(
                    citizen.Origin,
                    CitizenOriginSaveIds.AstralFounderId,
                    StringComparison.OrdinalIgnoreCase)
    ...
    public static WorldSave MigrateV30ToV31(WorldSave save)
    {
        ...
        if (save.Citizens.Count > 0)
        {
            save.FirstNight ??= new FirstNightSave
            {
                Stage = FirstNightStageSaveIds.ConcludedId,
        ...
        foreach (ExpeditionSave expedition in save.Expeditions)
        {
            ...
            bool isActiveFounderSpiritTrail = string.Equals(
                ...
                && (string.Equals(
                    ...
                    || (string.Equals(
                            expedition.Phase,
                            ExpeditionPhaseSaveIds.EncounterId,
                            StringComparison.OrdinalIgnoreCase)
                        && string.IsNullOrEmpty(expedition.EncounterOutcome)));
            ...
            if (founder.EquipmentLoadout.Weapon is null)
            {
                ...
                {
                    Family = WeaponFamilySaveIds.ToId(weapon.Family),
    ...
    public static WorldSave MigrateV33ToV34(WorldSave save)
    {
        ...
        foreach (ExpeditionSave expedition in save.Expeditions)
        {
            ...
            if (principalFounder?.EquipmentLoadout?.Weapon is { } savedWeapon)
            {
                ...
                if (string.Equals(
                        savedWeapon.Family,
                        WeaponFamilySaveIds.ToId(baseline.Family),
                        StringComparison.OrdinalIgnoreCase)
            ...
            if (expedition.ReservationId is int reservationId)
            {
                save.ResourceReservations.RemoveAll(reservation =>
                    ...
                    && string.Equals(
                        ...
                        ResourceReservationOwnerKindSaveIds.ToId(ResourceReservationOwnerKind.Expedition),
                        StringComparison.OrdinalIgnoreCase)
            ...
            expedition.RewardKind = ExpeditionRewardKindSaveIds.DiscoveryId;
            ...
            bool retreatTriggered = string.Equals(
                    expedition.RetreatPosture,
                    ExpeditionRetreatPostureSaveIds.ToId(ExpeditionRetreatPosture.RetreatAfterSetback),
                    StringComparison.OrdinalIgnoreCase)
                && string.Equals(
                    expedition.EncounterOutcome,
                    ExpeditionEncounterOutcomeSaveIds.ToId(ExpeditionEncounterOutcome.Setback),
                    StringComparison.OrdinalIgnoreCase);
            if (!retreatTriggered
                && string.Equals(
                    expedition.Phase,
                    ExpeditionPhaseSaveIds.ToId(ExpeditionPhase.Returning),
                    StringComparison.OrdinalIgnoreCase))
            {
...
#pragma warning restore CS0618
    ...
    private static string NormalizeAffinityForV29(string? value) =>
        string.IsNullOrWhiteSpace(value)
            ...
            || string.Equals(value, "neutral", StringComparison.OrdinalIgnoreCase)
                ? ElementalAffinityIdSaveIds.ToId(ElementalAffinityId.Silence)
        ...
        ResourceOpportunityKind kind)
    {
        ...
        save.ResourceOpportunities.Add(new ResourceOpportunitySave
        {
            Id = id,
            Kind = kind.ToString(),
            State = ResourceOpportunityStateSaveIds.AvailableId,
        });
    }
    ...
    public static WorldSave MigrateV18ToV19(WorldSave save)
    {
        ...
        foreach (ParcelSave parcel in save.Parcels)
        {
            parcel.TerritoryState = parcel.IsUnlocked
                ? ParcelTerritoryStateSaveIds.AvailableId
                : ParcelTerritoryStateSaveIds.LockedId;
        }
        if (save.Parcels.All(parcel => parcel.IsUnlocked))
        {
            ...
            save.Parcels.Add(new ParcelSave
            {
                ...
                LogicalRow = 0,
                IsUnlocked = false,
                TerritoryState = ParcelTerritoryStateSaveIds.LockedId,
            });
        }
    ...
    public static WorldSave MigrateV10ToV11(WorldSave save)
    {
        ...
        if (wood > 0)
        {
            save.CityInventory.TryGetValue(ResourceTypeSaveIds.WoodId, out int existing);
            save.CityInventory[ResourceTypeSaveIds.WoodId] = checked(existing + wood);
        }
        save.Version = 11;
        return save;
    }
