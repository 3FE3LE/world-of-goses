...
public class DayNightProductionTests
{
    ...
    public void AdvanceWorldTick_AtNight_WorkersRestAndRecover()
    {
        ...
        Assert.True(bran.CurrentStamina > branBefore,
            $"Expected Bran to recover at night (was {branBefore}, now {bran.CurrentStamina}).");
    }
    ...
    public void AdvanceWorldTick_AtNight_NoStaminaCost()
    {
        ...
        // night). Bran started night at whatever he had; he never
        // drains further.
    ...
    {
        // Load enough food so Bran eats every tick.
        var world = new CityWorld();
        ...
        Assert.True(bran.WellFedRemainingTicks > 0,
            $"Bran should have refreshed buff after eating (was {bran.WellFedRemainingTicks}).");
    }

    [Fact]
    public void AdvanceWorldTick_BuffDecaysAcrossTicks()
    {
        // Disable the Farm so Bran can never eat (and therefore
