...
if (-not (Get-Command gh -ErrorAction SilentlyContinue)) {
    Write-Host ""
    Write-Host "GitHub CLI (gh) no está instalado." -ForegroundColor Yellow
    Write-Host "En Windows puedes instalarlo con:" -ForegroundColor Yellow
    Write-Host "  winget install --id GitHub.cli" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "Después abre una terminal nueva, ejecuta 'gh auth login' y vuelve a correr este script." -ForegroundColor Yellow
    exit 1
}

gh auth status 2>$null | Out-Null
if ($LASTEXITCODE -ne 0) {
    Write-Host "gh está instalado pero no autenticado. Ejecuta: gh auth login" -ForegroundColor Yellow
    exit 1
}

Write-Host "Repo: $Repo" -ForegroundColor Cyan
Write-Host "Leyendo issues existentes para evitar duplicados..." -ForegroundColor Cyan

$existing = @(gh issue list --repo $Repo --state all --limit 500 --json number,title | ConvertFrom-Json)
$map = @()

function New-WogIssue {
    param(
    ...
    if ($found) {
        Write-Host "SKIP  #$($found.number)  $Title" -ForegroundColor DarkGray
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "No se pudo crear: $Title"
    }

    $number = [int]([regex]::Match($url, '/issues/(\d+)$').Groups[1].Value)
    Write-Host "CREATE #$number  $Title" -ForegroundColor Green

    $script:existing += [pscustomobject]@{ number = $number; title = $Title }
    $script:map += [pscustomobject]@{
        Number = $number
        Title = $Title
        Status = "created"
...
'@
New-WogIssue -Title "[EG-5C] Consolidar agricultura, parcelas 2\u20133 y segundo ciclo jugable" -Body $body -Label "enhancement"
...
'@
New-WogIssue -Title "[EG-6] Calibrar y firmar la apertura completa" -Body $body -Label "enhancement"
...
'@
New-WogIssue -Title "[Audio] Integrar arquitectura de reproducci\u00f3n y primer paquete sonoro" -Body $body -Label "enhancement"
...
'@
New-WogIssue -Title "[Territory] Dise\u00f1ar expansi\u00f3n territorial sobre el macro actual" -Body $body -Label "question"
...
'@
New-WogIssue -Title "[Performance] Revalidar culling y batching del Macro antes de ampliar territorio" -Body $body -Label "enhancement"
...
'@
New-WogIssue -Title "[RPG] Cerrar curva de competencias y coste de progresi\u00f3n 0\u201320" -Body $body -Label "enhancement"
...
'@
New-WogIssue -Title "[Combat] Completar contrato de t\u00e9cnicas, cr\u00edtico, penetraci\u00f3n y resistencias" -Body $body -Label "enhancement"
...
'@
New-WogIssue -Title "[Equipment] Extender PersonalEquipment a armadura, masa, integridad y desgaste" -Body $body -Label "enhancement"
...
'@
New-WogIssue -Title "[Health] Derivar ConditionFactor desde la condici\u00f3n real del Citizen" -Body $body -Label "enhancement"
...
'@
New-WogIssue -Title "[City/RPG] Derivar CitySupportFactor desde infraestructura y servicios" -Body $body -Label "enhancement"
...
'@
New-WogIssue -Title "[Elements] Integrar afinidades con materiales, ambiente, ciudad y expediciones" -Body $body -Label "enhancement"
...
'@
New-WogIssue -Title "[Combat] Definir escalado de bestiario y dificultad expedicionaria" -Body $body -Label "enhancement"
...
'@
New-WogIssue -Title "[NPC] Conectar DialogueRunner a un primer di\u00e1logo ramificado persistente" -Body $body -Label "enhancement"
...
'@
New-WogIssue -Title "[Citizens] Dise\u00f1ar envejecimiento, conocimiento y generaciones" -Body $body -Label "question"
...
'@
New-WogIssue -Title "[World] Cerrar cosmolog\u00eda com\u00fan de Ravatha" -Body $body -Label "question"
...
'@
New-WogIssue -Title "[Environment] Definir nombre y contrato del eje regeneraci\u00f3n/extracci\u00f3n" -Body $body -Label "question"
...
'@
New-WogIssue -Title "[World] Dise\u00f1ar primer bioma autoral y primer conflicto sist\u00e9mico" -Body $body -Label "question"
...
'@
New-WogIssue -Title "[City Design] Definir el siguiente estrato social: migraci\u00f3n, cultura, pol\u00edtica, econom\u00eda y capacidad" -Body $body -Label "question"
...
'@
New-WogIssue -Title "[Performance] Escalar representaci\u00f3n visual de citizens s\u00f3lo cuando el profiler lo exija" -Body $body -Label "enhancement"


$map = $map | Sort-Object Number
$map | Format-Table -AutoSize

$outPath = Join-Path (Get-Location) "world-of-goses-backlog-issue-map.json"
$map | ConvertTo-Json -Depth 4 | Set-Content -Path $outPath -Encoding UTF8

Write-Host ""
Write-Host "Mapa guardado en: $outPath" -ForegroundColor Cyan
Write-Host "Backlog migrado. Ya puedes usar GitHub Issues como única cola accionable." -ForegroundColor Green
