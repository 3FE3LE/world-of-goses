...
| 2026-08-15 | `typography-pixel-perfect` | 1280×720, 1920×1080 | **Pass, signed by eye.** Typography system replaced: Geist Pixel and Pixelify Sans out; Jacquard 24 (48/40/36), Jacquard 12 (20), Jacquarda Bastarda 9 (32), Jersey 15 (26/22), Jersey 10 (14-20) and Micro 5 (22/11) in. Both frames report a two-colour title crop, and the rebuilt specimen exercises all six families with `ÁÉÍÓÚ áéíóú Ññ Üü ¿? ¡!` and numerals: every stroke is solid, no grey column anywhere, and the 1.5x frame holds. The live HUD frame committed with this session's snapshot (`docs/session-state/2026-08-15-macro-1280x720.png`) signs Micro 5 at 22 px in real 24 px rows — the status, resource and construction rows all fit. Two reservations: Micro 5 sets noticeably wider than Pixelify did, so long resource names sit close to the panel edge; and Jacquard 12 at 20 px is the densest row in the set, acceptable for the short `HudBrand` logotype but not for anything longer. Still owed: `HudBadge` with a real non-zero count (no badge was visible in the live frame), and the founder ceremony surface — `FounderName` itself is signed in the specimen, but whether a long blackletter name fits `FounderCardPanel.CardWidth` is not. |

Two harness defects were fixed on the way to that row and are worth keeping on their own. `Capture-TypographySpecimen.ps1` never passed `--windowed` and passed the scene path after it rather than before; and `TypographySpecimen` shot on a fixed frame count, so on a 2560×1440 desktop it silently wrote a 2560×1440 image into a file named `1280x720`. It now pins its own window through `DisplayServer` — the same thing `VisualRegressionHarness.ApplyCaptureWindowSize` does, and for the reason already documented there — and waits in elapsed seconds rather than frames, because this scene renders thousands of frames per second and a frame budget that looked like ten seconds expired in a fraction of one.

`Capture-VisualMatrix.ps1` still fails intermittently on this machine with `Godot client settled at 960x480, expected 1280x720`, reproducible with no fixture and on unmodified scenes. It succeeded once during this session's snapshot and failed six consecutive times afterwards, including with a 6 s startup delay and a 25 s settle window. The script measures the Win32 client rect while the engine reports the window it pinned; asking the display server instead, as the specimen now does, is the likely fix. That is a harness change touching every visual fixture and belongs in its own issue.
