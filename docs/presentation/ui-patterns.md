...
> bible (`docs/systems/`); this file owns the
...
## 10. The expedition live surface

The rules a new screen inherits are §2–§9; this section records what the
expedition surfaces already commit to.

- `ExpeditionLiveView` presents the expedition over the same world clock; it
...
- Active Skills show four **octagonal** slots. Only Skill 1 is wired;
  actions are reserved as `expedition_skill_1` through
  `expedition_skill_4`. Basic Attack remains automatic.
- Each octagon keeps eight readable sides so a side-associated Trait has
  somewhere to live; no Trait or Chain UI exists.
- There is no `SPACE` action, carriage control or advanced formation control in
...
- `ExpeditionSquadStrip.tscn` and `ExpeditionSkillStrip.tscn` always author four
  ...
  slots are presentation only: they do not change
