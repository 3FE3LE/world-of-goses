...
| Chronicle | One `ChroniclePanel` whose header and body are mounted separately by `ExpeditionRail`: compact mode shows four rows; expanded mode shows the 80 newest compacted events, offline summary and actionable blocker groups. Both headers stay visible; the expedition and chronicle bodies share one `AccordionHost` that shows exactly one, which is the rail's only vertically expanding child. Closing Chronicle symmetrically restores the expedition body and `VER`, and the round-trip fixture asserts the body's real height against the card height rather than mere visibility — the previous visibility-only check passed on a body squeezed to 2 px. `ChronicleEventProjection` remains the single filtering/compaction rule, so routine resource gains stay excluded. The former `OfflineReportPanel` surface is removed. |
...
| 2026-08-10 | Spatial `ExpeditionStage` and Chronicle → expedition rail round-trip | Runtime/tests signed for snapshot ownership, integer interpolation and accordion restoration. Fresh 1280×720 / 1920×1080 graphical signature remains pending because both matrix attempts attached to the guarded 50×50 bootstrap client. |
| 2026-08-13 | `astral-weapon-choice` fixture (closes the visual sign-off for the Founder weapon materialization beat) | Captured at 1280×720 and 1920×1080 with `WeaponFamily` `Whip`/`Gauntlets` rendered as the two natural Caelith options, the first pre-selected with the `ButtonPrimary` palette and check glyph, confirm button enabled, footer contained. Hero profile snapshot surfaces the same family on `EquippedWeaponName`. The harness previously classified `astral-*` as `Other` and `AstralOnboardingView.ShowForVisualRegression` could not reach `Stage.WeaponChoice`; both seams are now wired so a future session can re-run the capture against an updated view without re-opening the harness. |
| 2026-08-13 | `macro-resource-placement-alignment` fixture (closes the visual sign-off for the placement/grid alignment bug) | Captured at 1280×720 and 1920×1080 with the Basic Shelter placement overlay open over the opening resources. The availability underlay renders one uniform strip per frontage cell, not three depth sub-cells. The resource asset and the grid cell that marks it as `NaturalResource` share the same lateral coordinate (via `MacroGroundProjection.ResourceAnchor`), so a reviewer can match every visible asset to its strip without a ruler. `MacroGroundProjection` is the single primitive resource, building and placement cells read from; the old per-site copy of the projection formula is gone. |
