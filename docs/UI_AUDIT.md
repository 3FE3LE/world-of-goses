...
| Expedition live perspective | One authored `ExpeditionLiveView` instance lives under `GameUiShell/ScreenContent`. A real active expedition exposes `VER` in `ExpeditionRail`; selecting it hides Macro, both side summaries, primary/context/action docks and the rail without freeing them. `CityStatusPanel` remains global. The view projects route, real members, health/stamina when available and real progress; missing encounter threat/enemy data is named as unavailable. AUTO/RETIRADA and the lateral stage are visual structure only, with no combat resolver or private clock. |
