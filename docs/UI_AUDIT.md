...
| Reusable controls | `StandardButtons`, `PanelHeader`, `AssignmentRow`, `SafeAreaMarginContainer` and shared theme variations. The future expedition-live seam now has four isolated PackedScenes: a real pixel-octagonal `OctagonalSkillSlot` with five states and eight invisible side anchors, `ExpeditionSquadSlot`, and four-child squad/skill strips. They are presentation-only and are not connected to a live screen or to `ExpeditionRequest.MaxTeamSize`. |
